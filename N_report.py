...
This script allows the user to generate human-readable reports based on the validation data produced by 'M_validate.py'.
The reports provide insights into the accuracy of a search algorithm by analyzing various statistics and metrics.
...
Usage:
Run this script from the console to select a specific validation file from available validations (generated by 'M_validate.py').
...
def generate_report(validation_file):
    ...
    Outputs:
        Displays a comprehensive report on various aspects of the validation data, providing insights into the search algorithm's accuracy.
    """
...
"""
    ...
    all_results_count = 0
    for d in raw_data:
        all_results_count = all_results_count + d["results_count"]
    ...
    # let's see how many results were given in the case of correct matches, average results count per match, the most repeated bad results
    ...
    slst = list(lst[:100])  # sliced copy of lst
    ...
    report_str = (
        report_str
        + "\n\n"
        + f"The {len(slst)} most frequently occurring incorrect entries were suggested {format(bcount, ',d')} times. \n"
        + f"If these {len(slst)} entries were removed from phrases.json, \n"
        + f"results provided for correct matches would total {format(total_results_count - bcount, ',d')} with an average of {round((total_results_count - bcount) / correct_match_count)} results per match."
        + "\n\n"
    )
...
while True:
    ...
    if user_input == "q":
        break
    elif user_input not in [str(x) for x in range(1, len(validation_files) + 1)]:
        continue
    else:
